...
class FavoritesViewModel : ObservableObject, Observable {
    ...
    init() {
        // bug with overlapping ids in favorites
        cityManager.setInternalIds(items: &cities, modifier: 100)
        hobbyManager.setInternalIds(items: &hobbies, modifier: 200)
        bookManager.setInternalIds(items: &books, modifier: 300)

        cities = cityManager.initializeFavorites(items: cities)
        hobbies = hobbyManager.initializeFavorites(items: hobbies)
        books = bookManager.initializeFavorites(items: books)
